...
# Глобальная переменная для хранения последней монеты (без таймфрейма)
_last_coin = None
...
async def produce_and_send_signal(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Фильтруем кандидатов, исключая последнюю монету (независимо от таймфрейма)
        filtered_candidates = [c for c in candidates if c.symbol != _last_coin]

        if not filtered_candidates:
            if chat_id and not quiet:
                await context.bot.send_message(chat_id=chat_id, text="Нет новых монет для сигнала. Жду следующего интервала...")
            return
